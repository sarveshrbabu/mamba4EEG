...
class MambaMixer(nn.Module):
    ...
    '''
    rewrote this function to fix the error with the time step projection
    no more nonsense with the self.dt_proj directly -> we have to set it to none if its not there
    '''
    def cuda_kernels_forward(self, hidden_states: torch.Tensor, cache_params=None):
        ...
        if self.training and cache_params is None:  # Doesn't support outputting the states -> used for training
            ...
            contextualized_states = mamba_inner_fn(
                ...
                self.D.float(),
                delta_bias=time_proj_bias if time_proj_bias is not None else torch.zeros_like(projected_states[..., 0]),
                delta_softplus=True,
            )
            ...
            if cache_params is not None and cache_params.seqlen_offset > 0:
                scan_outputs = selective_state_update(
                    ...
                    self.D,
                    gate[..., 0],
                    time_proj_bias if time_proj_bias is not None else torch.zeros_like(discrete_time_step[..., 0]),
                    dt_softplus=True,
                ).unsqueeze(-1)
            else:
